...
class JpaSetupSpec extends Specification {
    ...
    void "test setup entity manager with validation"() {
        ...
        em.flush()
        // TODO: Seems like validation is not performed so throw manually for test to pass
...
class BookService {
    ...
    List<Book> listBooks() {
        def cb = session.getCriteriaBuilder();
        def cq = cb.createQuery(Book)
        cq.from(Book)
        def books = session.createQuery(cq).getResultList()
        return books
    }
